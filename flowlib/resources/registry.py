...
class ResourceRegistry(BaseRegistry[T]):
    ...
    def get(self, name: str, resource_type: str = ResourceType.MODEL, expected_type: Optional[Type] = None) -> Any:
    ...
    def get_sync(self, name: str, resource_type: str = ResourceType.MODEL, expected_type: Optional[Type] = None) -> Any:
        ...
        if expected_type is not None and not isinstance(resource, expected_type):
            raise TypeError(f"Resource '{name}' has type {type(resource).__name__}, expected {expected_type.__name__}")
