...
async def run_autonomous(
    ...
    **kwargs
    ...
    logger.info(f"Starting autonomous run for agent '{agent.name}' (task: {agent.state.task_id}), max_cycles={cycles_limit}")
    ...
    try:
        ...
        while cycle_count < cycles_limit:
            ...
            if agent._state_persister and agent.config.state_config and agent.config.state_config.auto_save:
                 try:
        ...
        return agent.state # Return the final state
        ...
        if agent.state and hasattr(agent.state, 'add_error'):
            agent.state.add_error(str(e))
