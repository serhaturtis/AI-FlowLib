...
class FileStatePersister(BaseStatePersister):
    ...
    def __init__(self, directory: str = "./states"):
        """Initialize file state persister.
        
        Args:
            directory: Directory to save states in
        """
        super().__init__("file_state_persister")
    ...
    async def _initialize_impl(self) -> None:
        ...
        logger.debug(f"Using state directory: {self.directory}")
    ...
    async def _save_state_impl(
        ...
        try:
            ...
            # Create file path
            file_path = os.path.join(self.directory, f"{state.task_id}.json")
            ...
            if metadata:
                metadata_path = os.path.join(self.directory, f"{state.task_id}.meta.json")
    ...
    async def _load_state_impl(
        ...
        try:
            # Create file path
            file_path = os.path.join(self.directory, f"{task_id}.json")
    ...
    async def _delete_state_impl(
        ...
        try:
            # Create file paths
            state_path = os.path.join(self.directory, f"{task_id}.json")
            metadata_path = os.path.join(self.directory, f"{task_id}.meta.json")
    ...
    async def _list_states_impl(
        ...
        try:
            ...
            state_files = [f for f in os.listdir(self.directory) if f.endswith(".json") and not f.endswith(".meta.json")]
