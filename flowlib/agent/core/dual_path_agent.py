...
from ..classification.flow import MessageClassifierFlow, MessageClassifierInput
from ..conversation.handler import DirectConversationHandler
from ..tasks.handler import TaskExecutionHandler
from ..conversation.flow import ConversationFlow
from ..shell_command.flow import ShellCommandFlow
from ...flows.results import FlowResult
from ...flows.base import Flow
from ..models.plan import PlanExecutionOutcome
from ..planning.models import PlanStep
from ...flows.registry import stage_registry
from ...core.context import Context
from pydantic import BaseModel
from ..reflection.models import StepReflectionInput, StepReflectionResult, PlanReflectionContext
from ...utils.formatting.conversation import format_execution_history
# Import ShellCommandOutput for type checking
from ..shell_command.flow import ShellCommandOutput

logger = logging.getLogger(__name__)


class DualPathAgent(AgentCore):
    """Agent with separate paths for conversation and tasks"""
    ...
    def __init__(
        ...
        super().__init__(config, task_description)
    ...
    async def _initialize_impl(self) -> None:
        ...
        self.register_flow(self._classifier)
        
        # Create conversation handler with ConversationFlow
        conversation_flow = self.flows.get("ConversationFlow")
    ...
    async def process_message(self, message: str) -> FlowResult:
        ...
        if not self.initialized:
            ...
            return FlowResult(status="ERROR", message="Agent not initialized")
    ...
    async def _handle_single_input(self, message: str) -> FlowResult:
        ...
        if not self.state.task_description:
            ...
            self.state.task_description = self.config.task_description if self.config else default_task
            ...
            if self._memory:
                ...
                try:
                    ...
                    if self.state.task_id:
                        ...
                        context_metadata = {"task_description": self.state.task_description}
                        await self._memory.create_context(
                            context_name=self.state.task_id,
                            metadata=context_metadata
                        )
                        logger.debug(f"Created memory context for task: {self.state.task_id}")
                ...
                relevant_memories = await self._memory.retrieve_relevant(
                    query=message, 
                    context=self.state.task_id, # Scope to current task/session
                    limit=3 
                )
        ...
        try:
            ...
            classification = await self._classifier.classify_message(classifier_input)
            ...
            if not classification.execute_task:
                ...
                if classification.task_description:
                    original_task = self.state.task_description
                    self.state.task_description = classification.task_description
                    logger.info(f"Using task description from classification: {self.state.task_description}")
                else:
                    # Fallback if LLM didn't generate task description
                    logger.warning("Classifier indicated TASK but didn't provide task_description. Using existing state or default.")
                    self.state.task_description = self.state.task_description or "Execute task based on user message."
                ...
                if plan_outcome.status == "SUCCESS" or plan_outcome.status == "NO_ACTION_NEEDED":
                    ...
                    error_msg = plan_outcome.error or "Unknown error"
                    task_result_summary = f"Task execution failed during planning or execution. Error: {error_msg}"
                ...
                if self._reflection:
                    try:
                        ...
                        plan_reflect_context = PlanReflectionContext(
                            ...
                            current_progress=self.state.progress
                        )
                        ...
                        self.state.progress = reflection_result.progress
                        if reflection_result.is_complete:
                            self.state.set_complete(reflection_result.completion_reason or "Completed after reflection.")
                    except Exception as reflect_err:
                        logger.error(f"Reflection failed: {reflect_err}", exc_info=True)
                        self.state.add_error(f"Reflection failed: {reflect_err}")
                else:
            ...
            if final_result and final_result.status == "SUCCESS" and hasattr(final_result.data, 'response'):
                 self.state.add_system_message(final_result.data.response)
            elif final_result:
            ...
            # self.state.add_system_message(f"I encountered an error: {str(e)}") 
            # await self._save_state() 
    ...
    def _get_formatted_history(self) -> List[Dict[str, str]]:
        ...
        num_user = len(self.state.user_messages)
        num_system = len(self.state.system_messages)
        for i in range(num_user):
            history.append({"role": "user", "content": self.state.user_messages[i]})
            if i < num_system:
                history.append({"role": "assistant", "content": self.state.system_messages[i]})
        return history
            
    async def _save_state(self) -> None:
        """Save the agent state if a persister is available."""
        if self._state_persister and self.state.task_id:
            try:
    ...
    async def execute_cycle(self, **kwargs) -> bool:
        ...
        if "message" in kwargs:
            ...
            return not self.state.is_complete # Continue if task not marked complete
        else:
        ...
        while True: # Loop until plan completes, fails, or needs replanning
            ...
            if not current_plan:
                ...
                try:
                    ...
                    new_plan = await self._planner.plan(state)
            ...
            try:
                ...
                parsed_input = await self._planner.generate_inputs(
                    state=state,
                ...
                logger.info(f"Generated inputs for flow '{step.flow_name}': {str(parsed_input.model_dump())[:100]}...")
                 ...
                 outcome_result = FlowResult(status="ERROR", message=f"Input generation failed: {input_gen_err}", flow_name=step.flow_name)
            ...
            try:
                ...
                flow_to_run = stage_registry.get_flow(step.flow_name)
                if not flow_to_run:
                ...
                if step_result and step_result.status == "SUCCESS":
                    ...
                    self.state.add_execution_result(
                        flow_name=step.flow_name,
                    ...
                    if self._reflection:
                        ...
                        try:
                            step_reflection = await self._reflection.step_reflect(step_reflect_input)
                    ...
                    error_message = f"Step {state.current_step_index + 1} failed. Status: {step_result.status if step_result else 'UNKNOWN'}. Message: {step_result.message if step_result else 'No result object'}."
                    ...
                    if self._reflection:
                        ...
                        if not step_result:
                            step_result = FlowResult(status="ERROR", message="Step failed before result object was created.", flow_name=step.flow_name)
                        step_reflect_input = StepReflectionInput(
                            task_description=state.task_description,
                            step_id=step.step_id,
                            step_intent=step.step_intent,
                            step_rationale=step.rationale,
                            flow_name=step.flow_name,
                            flow_inputs=parsed_input if parsed_input else BaseModel(), # Pass dummy if needed
                ...
                outcome_result = FlowResult(status="ERROR", message=str(step_err), flow_name=step.flow_name) 
                state.add_error(error_message)
                # Add failed execution attempt to history
                self.state.add_execution_result(
                    cycle=state.cycles, # Add cycle number
                    flow_name=step.flow_name,
